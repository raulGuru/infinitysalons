<link rel="stylesheet" media="all" href="<?php echo base_url('/assets/css/settings.css') ?>">
<section class="main-content main-content--grey">
    <section class="page-wrapper page-wrapper--horizont">

        <div class="box-container">
            <div class="box box--left">
                <div class="box__title">
                    Sales
                </div>
                <ul class="link-list">
                    <li>
                        <a class="link-list__link" href="/reports/invoices">Invoices</a>
                    </li>
<!--                    <li>
                        <a class="link-list__link" href="/provider/calendar">Payments</a>
                    </li>-->
                    <li>
                        <a class="link-list__link" href="/provider/company_details">Daily sales</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/reports/serviceSales">Service Sales</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/reports/staffSales">Sales by staff</a>
                    </li>
<!--                    <li>
                        <a class="link-list__link" href="/communication_config/staff">Staff Notifications</a>
                    </li>-->
                </ul>
            </div>
<!--            <div class="box box--left">
                <div class="box__title">
                    Point of Sale
                </div>
                <ul class="link-list">
                    <li>
                        <a class="link-list__link" href="/provider/paymentmethods">Payment Types</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/provider/taxes">Taxes</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/discounts">Discount Types</a>
                    </li>
                                        <li>
                                            <a class="link-list__link" href="/provider/sales_settings">Sales Settings</a>
                                        </li>
                </ul>
            </div>-->
<!--            <div class="box box--left">
                <div class="box__title">
                    Client Settings
                </div>
                <ul class="link-list">
                                        <li>
                                            <a class="link-list__link" href="/communication_config/clients">Client Notifications</a>
                                        </li>
                    <li>
                        <a class="link-list__link" href="/provider/referral_sources">Referral Sources</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/provider/cancellations">Cancellation Reasons</a>
                    </li>
                    <li>
                        <a class="link-list__link" href="/user/permissions">Staff Permissions</a>
                    </li>
                </ul>
            </div>-->
        </div>
    </section>
</section>